...
def main():
    path = Path("../data/raw/IMF_WEO_dataset.csv")
    raw = load_raw_weo(path)
    panel = build_weo_panel(raw)

    panel.to_csv("../data/processed/weo_panel.csv", index=False)
    print("Saved processed dataset to data/processed/weo_panel.csv")
